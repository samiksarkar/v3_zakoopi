...
/*						ob_start();
						print_r($credentials);
						$sam=ob_get_contents();
						mail("saurabh@aphroecs.com","SamArray",$sam);
...
*/
				...
				$this->auth['info']['email'] =$credentials['screen_name'];
				$this->auth['info']['gender'] =5;
				
				
				if (!empty($credentials['screen_name'])) $this->auth['info']['nickname'] = $credentials['screen_name'];
				if (!empty($credentials['name'])) $this->auth['info']['first_name'] = $credentials['name'];
				if (!empty($credentials['location'])) $this->auth['info']['location'] = $credentials['location'];
				if (!empty($credentials['url'])) $this->auth['info']['website'] = $credentials['url'];
				if (!empty($credentials['url'])) $this->auth['info']['website'] = $credentials['url'];
